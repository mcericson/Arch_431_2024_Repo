...
def user_settings():
    ...
    save_model = rs.GetString("Would you like to save an obj file?", "No", ["Yes", "No"])
    return cube_dim, cursor, stop, save_animation, save_model



def main():
    rs.EnableRedraw(False)
    cube_dim, cursor, stop, save_animation, save_model = user_settings()
    vt.create_parallel_view("rot_axo", (800,800))
    points = cubic_grid(cube_dim, cube_dim, cube_dim, 1)
    rs.ZoomExtents()
    cubes = rgb_cube(points, 1)
    frames = 0
    ...
    if save_model == 'Yes':
        save_obj(cubes, "cube.obj", "ericson_cube")
